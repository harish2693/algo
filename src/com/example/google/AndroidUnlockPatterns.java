...
public class AndroidUnlockPatterns {
    ...
    public static int numberOfPatterns(int m, int n) {
        ...
        for(int i = m; i <= n; ++i) {
            //1, 3, 7, and 9 are symmetric so multiply this result by 4
            totalNumber += dfs(visited, skip, 1, i - 1) * 4;

            //2, 4, 6, and 8 are symmetric so multiply this result by 4
            totalNumber += dfs(visited, skip, 2, i - 1) * 4;

            //do not multiply by 4 because 5 is unique
            totalNumber += dfs(visited, skip, 5, i - 1);
        }

        return totalNumber;
    }

    private static int dfs(boolean[] visited, int[][] skip, int current, int remaining) {
        //base cases
        if(remaining < 0) {
            return 0;
        }
